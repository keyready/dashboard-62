...
const LoginForm = memo((props: LoginFormProps) => {
    const { className, onSuccess, type = 'login', onRegisterRedirect } = props;
    ...
    return (
        <DynamicModuleLoader removeAfterUnmount reducers={initialReducers}>
            ...
            >
                {error && (
                    <Alert className={classes.alert} style={{ margin: 0 }} variant="warning">
                        <p>{`Во время авторизации произошла ошибка: ${error}`}</p>
                        {type !== AuthType.REGISTER && <p>Нет аккаунта? Зарегистрируйтесь</p>}
                    </Alert>
                )}
                <Form.Control
