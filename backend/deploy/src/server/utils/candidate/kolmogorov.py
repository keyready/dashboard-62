...
def kolmogorov_test(data):
    ...
    # Проверка гипотезы
    if d > critical_value:
        print("Гипотеза отвергнута: выборка не соответствует теоретическому распределению.")
    else:
        print("Гипотеза принята: выборка соответствует теоретическому распределению.")